...
function DataGridPagination(props: DataGridPaginationProps) {
  ...
  // Ensure current pageSize is always present in selectable sizes
  const availableSizes = Array.from(
    new Set([...(mergedProps?.sizes || [10, 20, 50, 100]), pageSize]),
  ).sort((a, b) => a - b);
        ...
        {isLoading ? (
          ...
          <>
            <div className="text-xs sm:text-sm text-muted-foreground font-medium">
              {mergedProps?.sizesDescription || 'Rows per page'}
            </div>
            <Select
              value={`${pageSize}`}
            ...
            >
              <SelectTrigger className="w-fit min-w-[70px] h-8 text-xs font-semibold" size="sm">
