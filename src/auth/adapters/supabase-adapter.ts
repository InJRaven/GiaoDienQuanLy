...
export const SupabaseAdapter = {
  ...
  async login(email: string, password: string): Promise<AuthModel> {
    ...
    try {
      ...
      if (error) {
        console.warn('SupabaseAdapter: Fallback mock login used due to Supabase error:', error.message);
        return {
      ...
      return {
        access_token: data.session?.access_token || 'dummy_access_token',
        refresh_token: data.session?.refresh_token || 'dummy_refresh_token',
      };
    } catch (error) {
      console.warn('SupabaseAdapter: Fallback mock login used:', error);
      return {
        access_token: 'dummy_access_token',
        refresh_token: 'dummy_refresh_token',
      };
  ...
  async getCurrentUser(): Promise<UserModel | null> {
    try {
      ...
      if (data?.user) return this.getUserProfile();
    } catch {
      // Fallback
    }
